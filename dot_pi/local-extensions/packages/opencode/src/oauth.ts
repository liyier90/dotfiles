...
import { PSEUDO_OAUTH_EXPIRY_MS } from "./constants";
...
export function getPseudoOAuthApiKey(credentials: OAuthCredentials): string {
	if (
		...
		throw new Error(
			"Stored OpenCode credentials are missing the API key. Please /login again.",
		);
	}
...
export function createApiKeyBackedOAuthProvider(options: {
	...
	return {
		...
		async login(_callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
			return createPseudoOAuthCredentials("public");
		},
