...
export async function fetchOfficialModelEntries(
	url: string,
...
export function resolveModelDef(
	...
	return {
		...
		contextWindow:
			normalizePositiveNumber(metadata?.limit?.context) ??
			DEFAULT_CONTEXT_WINDOW,
		maxTokens:
			normalizePositiveNumber(metadata?.limit?.output) ?? DEFAULT_MAX_TOKENS,
	};
}
...
export function uniqueOfficialEntries(
	entries: OpenCodeModelListEntry[],
): OpenCodeModelListEntry[] {
...
export async function loadProviderBuckets(options: {
	...
	try {
		...
		console.warn(
			`[opencode] Failed to fetch ${options.label} models from ${options.officialEndpoint}`,
			error,
		);
	}
